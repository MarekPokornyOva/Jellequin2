namespace Tvl.VisualStudio.Language.Jellequin.Project
{
    ...
    [ComVisible(true)]
    public class JellequinProjectNode : ProjectNode
    {
        private static readonly char[] charsToEscape = new char[] { '%', '*', '?', '@', '$', '(', ')', ';', '\'' };

        private readonly JellequinBuildOptions _sharedBuildOptions;
        private VSLangProj.VSProject _vsProject;
        private ImageHandler _extendedImageHandler;

        public JellequinProjectNode(ProjectPackage package)
            : base(package)
        {
            _sharedBuildOptions = new JellequinBuildOptions();
        ...
        public JellequinBuildOptions SharedBuildOptions => _sharedBuildOptions;
        ...
        public override Guid ProjectGuid
        {
            get
            {
                ...
                return typeof(JellequinProjectFactory).GUID;
            }
        }

        public override string ProjectType => Constants.JellequinLanguageName;

        protected override bool SupportsProjectDesigner => true;

        /// <summary>
        /// Gets an ImageHandler for the project node.
        /// </summary>
        public ImageHandler ExtendedImageHandler
            => _extendedImageHandler
                    ?? (_extendedImageHandler = new ImageHandler(typeof(JellequinProjectNode).Assembly.GetManifestResourceStream("Tvl.VisualStudio.Language.Java.Project.Resources.imagelis.bmp")));
        ...
        internal void UpdateFolderBuildAction(JellequinFolderNode folderNode, FolderBuildAction buildAction)
        {
        ...
        {
            return string.Equals(itemType, JellequinProjectFileConstants.SourceFolder, StringComparison.OrdinalIgnoreCase)
                || base.FilterItemTypeToBeAddedToHierarchy(itemType);
        }

        protected override ReferenceContainerNode CreateReferenceContainerNode()
        {
            return new JellequinReferenceContainerNode(this);
        }

        public override FileNode CreateFileNode(ProjectElement item)
        {
            return new JellequinFileNode(this, item);
        }

        public override FolderNode CreateFolderNode(string path, ProjectElement element)
        {
            return new JellequinFolderNode(this, path, element);
        }

        protected override ConfigProvider CreateConfigProvider()
        {
            return new JellequinConfigProvider(this);
        }
        ...
        {
            ProjectOptions result= base.CreateProjectOptions();
            return result;
        }

        protected override NodeProperties CreatePropertiesObject()
        {
            NodeProperties result= base.CreatePropertiesObject();
            return result;
        }

        public override string GetProjectProperty(string propertyName, _PersistStorageType storageType, bool resetCache)
        {
            string result= base.GetProjectProperty(propertyName, storageType, resetCache);
            return result;
        }

        public override string GetProjectProperty(string propertyName, _PersistStorageType storageType)
        {
            string result= base.GetProjectProperty(propertyName, storageType);
            return result;
        }
        ...
        public override bool IsCodeFile(string fileName)
        {
            ...
            if (extension.Equals(Constants.JellequinFileExtension, StringComparison.OrdinalIgnoreCase))
                return true;
        ...
        {
            if (string.Equals(buildItem.ItemType, JellequinProjectFileConstants.SourceFolder, StringComparison.OrdinalIgnoreCase))
                return true;
        ...
        static Guid[] _configurationIndependentPropertyPages = new Guid[0];
        protected override Guid[] GetConfigurationIndependentPropertyPages()
            => _configurationIndependentPropertyPages;

        static Guid[] _configurationDependentPropertyPages= new Guid[]
                 {
                    typeof(PropertyPages.JellequinApplicationPropertyPage).GUID,
                    //typeof(PropertyPages.JellequinBuildPropertyPage).GUID,
                    //typeof(PropertyPages.JellequinDebugPropertyPage).GUID,
                 };
        protected override Guid[] GetConfigurationDependentPropertyPages()
            => _configurationDependentPropertyPages;
        ...
        protected override void SetBuildProject(MSBuild.Project project)
        {
            /*StringBuilder sb = new StringBuilder();
            using (StringWriter sw = new StringWriter(sb))
                project.Save(sw);
            MSBuild.Project project2;
            using (XmlReader xr = XmlReader.Create(new StringReader(sb.ToString())))
                project2 = new MSBuild.Project(xr);*/

            //<Import Project="$(MSBuildToolsPath)\Microsoft.CSharp.targets" />

            //project.Imports
            /*string path = Path.Combine(Path.GetDirectoryName(typeof(JellequinProjectNode).Assembly.Location),"Jellequin.targets");
            Microsoft.Build.Execution.ProjectInstance pi=new Microsoft.Build.Execution.ProjectInstance(path);
            //project.Properties
            foreach (KeyValuePair<string,Microsoft.Build.Execution.ProjectTargetInstance> target in pi.Targets)
                project.Targets.Add(target);*/

            string path = Path.Combine(Path.GetDirectoryName(typeof(JellequinProjectNode).Assembly.Location), "Jellequin.targets");
            Microsoft.Build.Construction.ProjectImportElement pie=project.Xml.Imports.FirstOrDefault(x => x.Project.EndsWith("Jellequin.targets", StringComparison.OrdinalIgnoreCase));
            if (pie != null && pie.Project != path)
            {
                project.Xml.RemoveChild(pie);
                pie = null;
            }
            if (pie == null)
                project.Xml.AddImport(path);

            base.SetBuildProject(project);
        }
    }
}
