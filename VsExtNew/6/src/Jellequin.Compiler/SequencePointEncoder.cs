namespace System.Reflection.Metadata.Ecma335
{
	...
	internal class SequencePointsEncoder
	{
		public BlobBuilder Builder { get; }
		public int LocalSignatureRowId { get; }

		public SequencePointsEncoder(BlobBuilder builder, int localSignatureRowId)
		{
			Builder=builder;
			LocalSignatureRowId=localSignatureRowId;
		}
	...
	public struct SequencePoint
	{
		...
		private SequencePoint(DocumentHandle document, int offset, int startLine, int startColumn, int endLine, int endColumn, bool isHidden)
		{
			Document=document;
			Offset=offset;
			StartLine=startLine;
		...
		public static SequencePoint Create(DocumentHandle document, int offset, int startLine, int startColumn, int endLine, int endColumn)
		{
			if (!IsValidPoint(startLine,startColumn,endLine,endColumn))
				throw new InvalidOperationException("Only hidden sequence points have zero width ");

			return new SequencePoint(document, offset, startLine, startColumn, endLine, endColumn, false);
		}

		public static SequencePoint Hidden(DocumentHandle document, int offset)
		{
			return new SequencePoint(document, offset, HiddenLine, HiddenColumn, HiddenLine, HiddenColumn, true);
		}

		public static bool IsValidPoint(int startLine, int startColumn, int endLine, int endColumn)
		{
			return !(startLine==endLine&&startColumn==endColumn) && ((startLine<endLine) || ((startLine==endLine) && (startColumn<endColumn)));
		}
	}
}
...
private BlobIdx SerializeSequencePoints(int localSignatureRowId, ImmutableArray<SequencePoint> sequencePoints, Dictionary<DebugSourceDocument, int> documentIndex, out int singleDocumentRowId)
{
	if (sequencePoints.Length == 0)
	{
	...
	int num = -1;
	int num2 = -1;
	blobBuilder.WriteCompressedInteger((uint)localSignatureRowId);
	DebugSourceDocument debugSourceDocument = MetadataWriter.TryGetSingleDocument(sequencePoints);
	singleDocumentRowId = ((debugSourceDocument != null) ? this.GetOrAddDocument(debugSourceDocument, documentIndex) : 0);
	for (int i = 0; i < sequencePoints.Length; i++)
	{
		DebugSourceDocument document = sequencePoints[i].Document;
		if (debugSourceDocument != document)
		{
			int orAddDocument = this.GetOrAddDocument(document, documentIndex);
			if (debugSourceDocument != null)
			{
				blobBuilder.WriteCompressedInteger(0u);
			}
			blobBuilder.WriteCompressedInteger((uint)orAddDocument);
			debugSourceDocument = document;
		}
		if (i > 0)
		{
			blobBuilder.WriteCompressedInteger((uint)(sequencePoints[i].Offset - sequencePoints[i - 1].Offset));
		}
		else
		{
			blobBuilder.WriteCompressedInteger((uint)sequencePoints[i].Offset);
		}
		if (sequencePoints[i].IsHidden)
		{
			blobBuilder.WriteInt16(0);
		...
		else
		{
			...
			if (num < 0)
			{
				blobBuilder.WriteCompressedInteger((uint)sequencePoints[i].StartLine);
				blobBuilder.WriteCompressedInteger((uint)sequencePoints[i].StartColumn);
			}
			else
			{
				blobBuilder.WriteCompressedSignedInteger(sequencePoints[i].StartLine - num);
	...
	return this._debugHeapsOpt.GetBlobIndex(blobBuilder);
}
*/
#endregion hint1

#region hint2
//https://github.com/fsharp/fsharp/blob/master/src/absil/ilwritepdb.fs
...
//// NOTE: This doesnï¿½t actually handle all overloads.  It just picks first entry with right 
//// number of arguments.
...
//    // Note, if we canï¿½t delete it code will fail later
...
#region hint3
//https://github.com/dotnet/roslyn/blob/master/src/Compilers/Core/Portable/PEWriter/MetadataWriter.PortablePdb.cs
#endregion hint3