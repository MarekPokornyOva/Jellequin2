...
namespace SimpleHttpServer
{
	class Program
	{
		static void Main(string[] args)
		{
			...
			Assembly asm = Compile(GetFilePath("SimpleHttpServer.jell"),true);
			...
			//You can freely launch the JS app directly by "dotnet SimpleHttpServerJell.dll" command.
		}
		...
		static Assembly Compile(string path,bool debug)
		{
			...
			{
				Compiler.Compile(code.GetText(),new AssemblyName("SimpleHttpServerJell") { Version=new Version(1,2,3,4) },new CompilerOptions
				{
					FileKind=FileKind.ConsoleExe,
					RuntimeMethodsUsage=RuntimeMethodsUsage.Copy,
				...
				string basePath = GetFilePath(Path.Combine("out","SimpleHttpServerJell."));
				dll.Position=0;
